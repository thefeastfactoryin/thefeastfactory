const tffMenuImageFiles = [
  ...
  'Crispy baby Corn.avif', 'Curd Rice.avif', 'Dal Fry.avif', 'Dal Tadka.avif',
  'Double Ka Meetha.avif', 'Egg Bhurji.avif', 'Egg Biryani.avif',
  ...
  'Kadai Paneer.avif', 'Kaju Crispy corn.avif', 'Kaju Mushroom.avif',
  ...
  'Miriyala Mamsam Vepudu.avif', 'Mix-veg.avif', 'Mushroom Masala.avif',
  ...
  'Pachimirchi Paneer Pulao.avif', 'Palak Paneer.avif',
  ...
  'Prawns Vepudu.avif', 'Sambar Rice.avif', 'Tandoori Roti.avif',
...
const getMenuImageFolder = (filename: string) => {
  ...
  if (/rice|noodles/.test(normalized)) return 'rice-items';
  if (/naan|phulka|roti/.test(normalized)) return 'indian-breads';
  if (/jamun|meetha/.test(normalized)) return 'desserts';
  if (/curry|pulsu|pulusu|koora|kura|masala|handi/.test(normalized)) {
    return 'curry';
  }
  return 'starters';
};
